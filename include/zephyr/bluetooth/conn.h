 ...
 *                           Disabled by default.
 *  @param apr_enable        Enable or disable Acceptable Power Reduction (APR) handling
 ...
 *  flattening peaks, which also means the controller reacts slower
 *  on RSSI changes.
 ...
 *  For example, for gamma to be 0.25,
 *  set the beta parameter in the command to 1024.
 *  @endparblock
 *  @param lower_limit       The lower limit of the RSSI golden range. The RSSI golden range
 *                           is explained in Core_v5.4, Vol 6, Part B, Section 5.1.17.1.
 *                           Default value is -70 dBm.
 *  @param upper_limit       The upper limit of the RSSI golden range.
 ...
 *                           It is needed to not repeat send requests for transmit power change
 *                           without the remote having had the chance to react, as well as to avoid
 *                           a busy controller. This value should be set depending on needs.
 ...
 *  @retval -ENOBUFS HCI command buffer is not available.
 */
int bt_conn_set_power_control_request_params(uint8_t auto_enable,
					uint8_t apr_enable, uint16_t beta, int8_t lower_limit,
