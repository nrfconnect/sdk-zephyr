...
pipeline {
  ...
  environment {
      ...
      ARCH = "-a arm"
      SANITYCHECK_OPTIONS_ALL = "--inline-logs"
      SANITYCHECK_OPTIONS_NRF = "--board-root $WORKSPACE/nrf/boards --testcase-root $WORKSPACE/nrf/samples --build-only --disable-unrecognized-section-test -t ci_build --inline-logs -p nrf9160_pca10090"
  ...
  stages {
    ...
    stage('Install missing tools') {
      steps {
        // Upstream CI now requires PIL, without declaring this in the requirements.txt file.
        // Manually install here until it can be found in the Docker image.
        sh("pip3 install pillow --user")
      }
    }
    ...
    stage('Testing') {
      parallel {
        ...
        stage('Sanitycheck (nrf91)') {
          steps {
            dir('zephyr') {
              sh "source zephyr-env.sh && ./scripts/sanitycheck $SANITYCHECK_OPTIONS_ALL $ARCH -p nrf9160_pca10090"
            }
          }
        }

        stage('Build samples (nrf91)') {
          steps {
            dir('zephyr') {
