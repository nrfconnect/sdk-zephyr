...
#define PCLK_NODE DT_NODELABEL(fll16m)
#elif defined(CONFIG_SOC_NRF54L20)
#define PCLK_NODE DT_NODELABEL(pclk32m)
#else
#error "No valid PCLK clock source found. "
#endif
...
void tdm_irq_handler(const struct device *dev)
{
	...
	NRF_TDM_Type *p_reg = drv_data->p_reg;
	tdm_ctrl_t *ctrl_data = drv_data->control_data;
...
static bool get_next_rx_buffer(struct tdm_drv_data *drv_data, tdm_buffers_t *buffers)
{
	...
	ret = dmm_buffer_in_prepare(drv_data->mem_reg, buffers->p_rx_mem_slab,
...
{
	(void)dmm_buffer_out_release(drv_data->mem_reg, buf->dmm_buf);
...
{
	(void)dmm_buffer_in_release(drv_data->mem_reg, buf->mem_block, buf->size, buf->dmm_buf);
	...
	if (nrfx_cfg.mode == NRF_TDM_MODE_MASTER) {
		...
		const uint32_t src_freq =
			(drv_cfg->clk_src == ACLK)
				...
				: DT_PROP(PCLK_NODE, clock_frequency);
		...
		if (((nrf_tdm_mck_pin_get(drv_data->p_reg) & TDM_PSEL_MCK_CONNECT_Msk) ==
...
static int tdm_nrfx_read(const struct device *dev, void **mem_block, size_t *size)
{
	...
	if (ret == 0) {
		(void)dmm_buffer_in_release(drv_data->mem_reg, buf.mem_block, buf.size,
					    buf.dmm_buf);
...
static int tdm_nrfx_write(const struct device *dev, void *mem_block, size_t size)
{
	...
	ret = dmm_buffer_out_prepare(drv_data->mem_reg, buf.mem_block, buf.size,
				     (void **)&buf.dmm_buf);
		     ...
		     tdm_data_handler_t handler)
{
	...
	nrf_tdm_mck_set(drv_data->p_reg, p_config->mck_setup != 0);
...
static int trigger_start(const struct device *dev)
{
	...
	nrf_tdm_sck_configure(drv_data->p_reg,
			      drv_cfg->clk_src == ACLK ? NRF_TDM_SRC_ACLK : NRF_TDM_SRC_PCLK32M,
			      false);

	nrf_tdm_mck_configure(drv_data->p_reg,
			      drv_cfg->clk_src == ACLK ? NRF_TDM_SRC_ACLK : NRF_TDM_SRC_PCLK32M,
			      false);
...
#define TDM_NRFX_DEVICE(idx)                                                                       \
	...
	{                                                                                          \
		...
		IF_ENABLED(CONFIG_CLOCK_CONTROL_NRF, (init_clock_manager(dev);))                   \
		return 0;                                                                          \
	}                                                                                          \
	BUILD_ASSERT(TDM_CLK_SRC(idx) != ACLK, "Clock source ACLK is currently not supported. ");  \
