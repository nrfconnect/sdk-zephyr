...
{
	int ret = k_mem_slab_alloc(drv_data->rx.cfg.mem_slab, (void **)&buffers->p_rx_buffer,
				   K_NO_WAIT);
	if (ret < 0) {
...
static void free_tx_buffer(struct tdm_drv_data *drv_data, const void *buffer)
{
	k_mem_slab_free(drv_data->tx.cfg.mem_slab, (void *)buffer);
	LOG_DBG("Freed TX %p", buffer);
}

static void free_rx_buffer(struct tdm_drv_data *drv_data, void *buffer)
{
...
#warning check description below
		const uint32_t src_freq =
			(drv_cfg->clk_src == ACLK)
				...
				: DT_PROP(DT_NODELABEL(fll16m), clock_frequency);

		/* Unless the FLL16M source is used,
		 * it is required to request the proper clock to be running
		 * before starting the transfer itself.
		 */
		drv_data->request_clock = (drv_cfg->clk_src != FLL16M);
...
static void data_handler(const struct device *dev, const tdm_buffers_t *released, uint32_t status)
{
	...
	if (status & NRFX_TDM_STATUS_TRANSFER_STOPPED) {
		...
		if (drv_data->last_tx_buffer) {
			...
			if (drv_data->last_tx_buffer != released->p_tx_buffer) {
				free_tx_buffer(drv_data, drv_data->last_tx_buffer);
			}
	...
	if (released->p_rx_buffer) {
		if (drv_data->discard_rx) {
			...
			struct tdm_buf buf = {.mem_block = released->p_rx_buffer,
