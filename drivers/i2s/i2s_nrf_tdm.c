...
struct tdm_drv_data {
	...
	DT_NODE_HAS_STATUS_OKAY(DT_NODELABEL(aclk))
...
static void clock_manager_init(const struct device *dev)
{
	...
	drv_data->aclk_spec.frequency = DT_PROP(DT_NODELABEL(hfclkaudio), hfclkaudio_frequency);
#elif NRF_CLOCK_HAS_HFCLK24M
	struct tdm_drv_data *drv_data = dev->data;

	IF_ENABLED(NRF_CLOCK_HAS_HFCLK24M,
		   (drv_data->audioclock = DEVICE_DT_GET_ONE(nordic_nrf_clock_xo24m);))
	drv_data->aclk_spec.frequency = DT_PROP(DT_NODELABEL(xo24m), clock_frequency);
#else
	(void)dev;
