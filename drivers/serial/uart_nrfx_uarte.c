...
static void uarte_pm_suspend(const struct device *dev)
{
	...
	if (data->async) {
		...
		if (IS_ENABLED(CONFIG_PM_DEVICE_RUNTIME) || LOW_POWER_ENABLED(cfg)) {
			 ...
			 * If low power mode is enabled then UARTE shall be disabled.
			 */
		...
		if (LOW_POWER_ENABLED(cfg)) {
			/* If low power mode is enabled then UARTE is disabled when
			 * TX and RX are inactive. Only pinctrl state change is needed.
			 */
