 ...
 * used by other cores, treat the UART like a normal one. This presumes the CPUAPP or CPURAD
...
#define INSTANCE_IS_FAST(unused, prefix, idx, _)						\
	UTIL_AND(										\
		UTIL_AND(									\
			IS_ENABLED(_CONCAT_3(CONFIG_HAS_HW_NRF_UARTE, prefix, idx)),		\
 ...
 *
 * Requesting/releasing clocks or power domains is not neccesarily ISR safe (we can't
 * relyable now, its out of our control). UARTE_ON_MANAGED_POWER_DOMAIN() lets us check if we
