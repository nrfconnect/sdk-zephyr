...
static void *zsai_sim_buffer(const struct device *dev, off_t offset)
{
	return (void *)&(ZSAI_SIM_DEV_BUFFER(dev)[offset]);
}

static int in_range(const struct device *dev, off_t offset, size_t len)
{
	if (offset < 0 || (uint32_t)len > ZSAI_SIM_DEV_SIZE(dev)) {
		return 0;
	}
...
static int zsai_sim_read(const struct device *dev, void *data, const off_t offset,
			 const size_t len)
{
...
#if ZSAI_SIM_ERASE_SUPPORT_NEEDED
static int zsai_sim_erase(const struct device *dev, const off_t offset,
			  const size_t size)
{
			      ...
			      const uintptr_t in, uintptr_t in_out)
{
	...
	case ZSAI_IOCTL_DO_ERASE:
		if (ZSAI_ERASE_REQUIRED(dev)) {
			const struct zsai_ioctl_range *er =
				(const struct zsai_ioctl_range *)in;

			return zsai_sim_erase(dev, er->offset, er->size);
		}
		/* Break out to not supported */
		break;
#endif
	}
	return -ENOTSUP;
	...
	struct zsai_sim_dev_config zsai_sim_dev_config_##instance  = {				\
		.generic = {									\
			.infoword = ZSAI_MK_INFOWORD(						\
				...
				/* Existence erase-value determines need for erase */		\
