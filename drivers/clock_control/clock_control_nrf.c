			  ...
			  clock_control_subsys_rate_t rate)
{
	...
	uint32_t freq_hz = *(uint32_t *)rate;
	nrf_clock_hfclk_div_t div = UINT32_MAX;
	int ret;
	uint32_t *supported_frequencies = NULL;
	...
	} else if (sys == CLOCK_CONTROL_NRF_TYPE_HFCLK192M) {
	...
	if (sys == CLOCK_CONTROL_NRF_TYPE_HFCLK) {
		...
		ret = nrfx_clock_hfclk_divider_set(div);
		if (ret != 0) {
			return ret;
		}
	} else {
		...
		ret = nrfx_clock_hfclk192m_divider_set(div);
		if (ret != 0) {
			return ret;
		}
	}
...
static int hfclk_get_rate(const struct device *dev, clock_control_subsys_t sys,
			  clock_control_subsys_rate_t *rate)
{
	ARG_UNUSED(dev);

	if (sys == CLOCK_CONTROL_NRF_TYPE_HFCLK) {
		*rate = nrfx_clock_hfclk_divider_get();
	} else if (sys == CLOCK_CONTROL_NRF_TYPE_HFCLK192M) {
		*rate = nrfx_clock_hfclk192m_divider_get();
	} else {
		return -ENOSYS;
	}
