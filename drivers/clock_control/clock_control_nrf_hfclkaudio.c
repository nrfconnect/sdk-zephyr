...
#define CLOCK_DEVICE_HFCLKAUDIO DEVICE_DT_GET(DT_NODELABEL(hfclkaudio))
...
#define CLOCK_LOG(lvl, dev, ...)                                                                   \
	LOG_##lvl("%s: " GET_ARG_N(1, __VA_ARGS__),                                                \
		  "hfclkaudio" COND_CODE_0(NUM_VA_ARGS_LESS_1(__VA_ARGS__),\
...
{
	clock_control_cb_t callback = ((hfclkaudio_data_t *)dev->data)->cb;

	((hfclkaudio_data_t *)dev->data)->cb = NULL;
	set_on_state(&((hfclkaudio_data_t *)dev->data)->flags);
	DBG(dev, "Clock started");

	if (callback) {
		callback(dev, NULL, ((hfclkaudio_data_t *)dev->data)->user_data);
	}
}
...
static int stop(const struct device *dev, uint32_t ctx)
{
	...
	err = set_off_state(&((hfclkaudio_data_t *)dev->data)->flags, ctx);
	if (err < 0) {
		return err;
	}

	((hfclkaudio_config_t *)dev->config)->stop();
		       ...
		       uint32_t ctx)
{
	...
	err = set_starting_state(&((hfclkaudio_data_t *)dev->data)->flags, ctx);
	if (err < 0) {
		return err;
	}

	((hfclkaudio_data_t *)dev->data)->cb = cb;
	((hfclkaudio_data_t *)dev->data)->user_data = user_data;

	((hfclkaudio_config_t *)dev->config)->start();
...
static void onoff_stop(struct onoff_manager *mgr, onoff_notify_fn notify)
{
	...
	res = stop(CLOCK_DEVICE_HFCLKAUDIO, CTX_ONOFF);
	notify(mgr, res);
}
				   ...
				   void *user_data)
{
	...
	notify(&((hfclkaudio_data_t *)dev->data)->mgr, 0);
}
...
static void onoff_start(struct onoff_manager *mgr, onoff_notify_fn notify)
{
	...
	err = async_start(CLOCK_DEVICE_HFCLKAUDIO, onoff_started_callback, notify, CTX_ONOFF);
	if (err < 0) {
		notify(mgr, err);
	}
}
...
{
	const struct device *dev = CLOCK_DEVICE_HFCLKAUDIO;
...
{
#if DT_NODE_HAS_PROP(DT_NODELABEL(hfclkaudio), hfclkaudio_frequency)
	const uint32_t frequency = DT_PROP(DT_NODELABEL(hfclkaudio), hfclkaudio_frequency);
						...
						clock_control_subsys_t subsys)
{
	...
	return GET_STATUS(((hfclkaudio_data_t *)dev->data)->flags);
}

static int api_request(const struct device *dev, const struct nrf_clock_spec *spec,
		       struct onoff_client *cli)
{
	hfclkaudio_data_t *dev_data = dev->data;
...
static int clk_init(const struct device *dev)
{
	...
	err = onoff_manager_init(&((hfclkaudio_data_t *)dev->data)->mgr, &transitions);
	if (err < 0) {
		return err;
	}

	((hfclkaudio_data_t *)dev->data)->flags = CLOCK_CONTROL_STATUS_OFF;

	return 0;
}

CLOCK_CONTROL_NRF_IRQ_HANDLERS_ITERABLE(clock_control_nrf_hfclkaudio,
...
static hfclkaudio_data_t data;

static const hfclkaudio_config_t config = {
