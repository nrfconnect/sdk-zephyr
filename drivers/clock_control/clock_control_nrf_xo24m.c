...
static void onoff_stop(struct onoff_manager *mgr, onoff_notify_fn notify)
{
	...
	res = common_stop(CLOCK_DEVICE_XO24M, COMMON_CTX_ONOFF);
	notify(mgr, res);
}
...
static void onoff_start(struct onoff_manager *mgr, onoff_notify_fn notify)
{
	...
	err = common_async_start(CLOCK_DEVICE_XO24M, common_onoff_started_callback, notify,
				 COMMON_CTX_ONOFF);
	if (err < 0) {
		notify(mgr, err);
	}
		     ...
		     void *user_data)
{
	...
	return common_async_start(CLOCK_DEVICE_XO24M, cb, user_data, COMMON_CTX_API);
}
...
static int api_stop(const struct device *dev, clock_control_subsys_t subsys)
{
	...
	return common_stop(CLOCK_DEVICE_XO24M, COMMON_CTX_API);
}
						...
						clock_control_subsys_t subsys)
{
	...
	return COMMON_GET_STATUS(((common_clock_data_t *)CLOCK_DEVICE_XO24M->data)->flags);
}
		       ...
		       struct onoff_client *cli)
{
	...
	return onoff_request(&((common_clock_data_t *)CLOCK_DEVICE_XO24M->data)->mgr, cli);
}
...
static int api_release(const struct device *dev, const struct nrf_clock_spec *spec)
{
	...
	return onoff_release(&((common_clock_data_t *)CLOCK_DEVICE_XO24M->data)->mgr);
}
				 ...
				 struct onoff_client *cli)
{
	...
	return onoff_cancel_or_release(&((common_clock_data_t *)CLOCK_DEVICE_XO24M->data)->mgr,
				       cli);
}
...
static int clk_init(const struct device *dev)
{
	...
	err = onoff_manager_init(&((common_clock_data_t *)CLOCK_DEVICE_XO24M->data)->mgr,
				 &transitions);
	if (err < 0) {
		return err;
	}

	((common_clock_data_t *)CLOCK_DEVICE_XO24M->data)->flags = CLOCK_CONTROL_STATUS_OFF;

	return 0;
}

CLOCK_CONTROL_NRF_IRQ_HANDLERS_ITERABLE(clock_control_nrf_xo24m, &nrfx_clock_xo24m_irq_handler);
...
static const common_clock_config_t config = {
	.start = nrfx_clock_xo24m_start,
	.stop = nrfx_clock_xo24m_stop,
};

DEVICE_DT_DEFINE(DT_NODELABEL(xo24m), clk_init, NULL, &data, &config, PRE_KERNEL_1,
