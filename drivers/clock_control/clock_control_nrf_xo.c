...
static int async_start(clock_control_cb_t cb, void *user_data, uint32_t ctx)
{
	int err;

	err = set_starting_state(&((xo_data_t *)CLOCK_DEVICE_XO->data)->flags, ctx);
	if (err < 0) {
		return err;
	}

	((xo_data_t *)CLOCK_DEVICE_XO->data)->cb = cb;
	((xo_data_t *)CLOCK_DEVICE_XO->data)->user_data = user_data;

	((xo_config_t *)CLOCK_DEVICE_XO->config)->start();
...
static int stop(uint32_t ctx)
{
	int err;

	err = set_off_state(&((xo_data_t *)CLOCK_DEVICE_XO->data)->flags, ctx);
	if (err < 0) {
		return err;
	}

	((xo_config_t *)CLOCK_DEVICE_XO->config)->stop();
...
#endif
	clock_control_cb_t callback = ((xo_data_t *)CLOCK_DEVICE_XO->data)->cb;
	void *user_data = ((xo_data_t *)CLOCK_DEVICE_XO->data)->user_data;

	((xo_data_t *)CLOCK_DEVICE_XO->data)->cb = NULL;
	set_on_state(&((xo_data_t *)CLOCK_DEVICE_XO->data)->flags);
	DBG(CLOCK_DEVICE_XO, "Clock started");

	if (callback) {
		callback(CLOCK_DEVICE_XO, NULL, user_data);
	}
}

static void clkstarted_event_handle(void)
{
	if (GET_STATUS(((xo_data_t *)CLOCK_DEVICE_XO->data)->flags) == CLOCK_CONTROL_STATUS_STARTING) {
				   ...
				   void *user_data)
{
	...
	notify(&(((xo_data_t *)CLOCK_DEVICE_XO->data)->mgr), 0);
}
			...
			onoff_notify_fn notify)
{
	...
	err = async_start(onoff_started_callback, notify, CTX_ONOFF);
	if (err < 0) {
		notify(mgr, err);
	}
}
...
static void clock_event_handler(nrfx_clock_xo_event_type_t event)
{
	...
	case NRFX_CLOCK_XO_EVT_HFCLK_STARTED:
	{
		 ...
		 */
		if (GET_STATUS(((xo_data_t *)CLOCK_DEVICE_XO->data)->flags)
		     ...
		     clock_control_cb_t cb, void *user_data)
{
	...
	return async_start(cb, user_data, CTX_API);
}
			      ...
			      clock_control_subsys_t subsys)
{
	...
	err = api_start(NULL, NULL, blocking_start_callback, &sem);
	if (err < 0) {
		return err;
	}
					    ...
					    clock_control_subsys_t subsys)
{
	...
	return GET_STATUS(((xo_data_t *)CLOCK_DEVICE_XO->data)->flags);
}
		       ...
		       struct onoff_client *cli)
{
	...
	return onoff_request(&((xo_data_t *)CLOCK_DEVICE_XO->data)->mgr, cli);
}
		       ...
		       const struct nrf_clock_spec *spec)
{
	...
	return onoff_release(&((xo_data_t *)CLOCK_DEVICE_XO->data)->mgr);
}
				 ...
				 struct onoff_client *cli)
{
	...
	return onoff_cancel_or_release(&((xo_data_t *)CLOCK_DEVICE_XO->data)->mgr, cli);
}
...
static int clk_init(const struct device *dev)
{
	...
	err = onoff_manager_init(&((xo_data_t *)CLOCK_DEVICE_XO->data)->mgr,
					&transitions);
	if (err < 0) {
		return err;
	}
