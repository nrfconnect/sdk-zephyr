...
{
	clock_control_cb_t callback = ((hfclk_data_t *)CLOCK_DEVICE_HFCLK->data)->cb;

	((hfclk_data_t *)CLOCK_DEVICE_HFCLK->data)->cb = NULL;
	set_on_state(&((hfclk_data_t *)CLOCK_DEVICE_HFCLK->data)->flags);
	DBG(CLOCK_DEVICE_HFCLK, "Clock started");

	if (callback) {
		callback(CLOCK_DEVICE_HFCLK, NULL,
			 ((hfclk_data_t *)CLOCK_DEVICE_HFCLK->data)->user_data);
	}
}
...
static void clock_event_handler(void)
{
	 ...
	 */
	if (GET_STATUS(((hfclk_data_t *)CLOCK_DEVICE_HFCLK->data)->flags) ==
...
static int clk_init(const struct device *dev)
{
	...
	err = onoff_manager_init(&((hfclk_data_t *)dev->data)->mgr, &transitions);
	if (err < 0) {
		return err;
	}
...
static const hfclk_config_t config = {.start = generic_hfclk_start,
				      .stop = generic_hfclk_stop,
				      IF_ENABLED(CONFIG_LOG, (.name = "hfclk",)) };

DEVICE_DT_DEFINE(DT_NODELABEL(hfclk), clk_init, NULL, &data, &config, PRE_KERNEL_1,
