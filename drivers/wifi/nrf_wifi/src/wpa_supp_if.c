...
int nrf_wifi_wpa_set_supp_port(void *if_priv, int authorized, char *bssid)
{
	...
	int peer_id = nrf_wifi_fmac_peer_get_id(rpu_ctx_zep->rpu_ctx, chg_sta_info.mac_addr);
	if (peer_id == -1) {
			...
			unsigned int flags_and)
{
	...
	int peer_id = nrf_wifi_fmac_peer_get_id(rpu_ctx_zep->rpu_ctx, chg_sta.mac_addr);
	if (peer_id == -1) {
		nrf_wifi_osal_log_err("%s: Invalid peer",
				      __func__);
		goto out;
