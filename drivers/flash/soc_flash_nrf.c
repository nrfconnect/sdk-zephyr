			       ...
			       uintptr_t in_out)
{
	...
	case ZSAI_IOCTL_DO_ERASE: {
		const struct zsai_ioctl_range *eio = (const struct zsai_ioctl_range *)in;

#ifdef CONFIG_USERSPACE
		struct zsai_ioctl_range eio_local;

		if (syscall_trap) {
			Z_OOPS(z_user_from_copy(&eio_local, eio, sizeof(eio_local)));
			eio = &eio_local;
		}
#endif

		return flash_nrf_erase(dev, (off_t)eio->offset, (size_t)eio->size);
	}
...
#if defined(CONFIG_SOC_FLASH_NRF_EMULATE_ONE_BYTE_WRITE_ACCESS)
	ZSAI_MK_INFOWORD(1, true, 0xff, true),
#else
	ZSAI_MK_INFOWORD(4, true, 0xff, true),
#endif
};
