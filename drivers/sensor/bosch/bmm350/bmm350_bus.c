...
static int bmm350_prep_reg_read_rtio_async(const struct bmm350_bus *bus, uint8_t reg, uint8_t *buf,
...
static int bmm350_prep_reg_write_rtio_async(const struct bmm350_bus *bus, uint8_t reg, uint8_t val,
					    struct rtio_sqe **out)
{
	struct rtio *ctx = bus->rtio.ctx;
	struct rtio_iodev *iodev = bus->rtio.iodev;
	...
	rtio_sqe_prep_tiny_write(write_sqe, iodev, RTIO_PRIO_NORM, msg, sizeof(msg), NULL);
	if (bus->rtio.type == BMM350_BUS_TYPE_I2C) {
