			    ...
			    u16_t *val, bool send_stop)
{
	...
	if (i2c_transfer(drv_data->i2c, msgs, 2, DT_INST_0_TI_OPT3001_BASE_ADDRESS) != 0) {
		return -EIO;
	}
			      ...
			      u16_t mask, u16_t val)
{
	...
	if (opt3001_reg_read(drv_data, reg, &old_val, true) != 0) {
		return -EIO;
	}
...
static int opt3001_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	if (opt3001_reg_read(drv_data, OPT3001_REG_RESULT, &value,
			     true) != 0) {
		return -EIO;
	}
			       ...
			       struct sensor_value *val)
{
     ...
     *
     * lux is the integer obtained using the following formula: (2^(exponent value)) * 0.01 * mantisa value
	 */
	uval = (1 << (drv_data->sample >> OPT3001_SAMPLE_EXPONENT_SHIFT)) * (drv_data->sample & OPT3001_MANTISSA_MASK);
	val->val1 = uval / 100;
...
static int opt3001_chip_init(struct device *dev)
{
	...
	u16_t value;
	if (opt3001_reg_read(drv_data, OPT3001_REG_MANUFACTURER_ID, &value, true) != 0) {
		return -EIO;
	}
	...
	if (opt3001_reg_read(drv_data, OPT3001_REG_DEVICE_ID, &value, true) != 0) {
		return -EIO;
	}
