					  ...
					  size_t output_size, size_t *output_len)
{
	...
	ret = mbedtls_chachapoly_encrypt_and_tag(&ctx, input_len, nonce_buf, add_buf, add_len,
						 input_buf, data, tag);
	if (ret < 0) {
		goto cleanup;
	}
					  ...
					  size_t output_size, size_t *output_len)
{
	...
	if (nonce_len < 12 || input_len < 16 || output_size < (input_len - 16)) {
		return PSA_ERROR_INVALID_ARGUMENT;
	}
	...
	ret = mbedtls_chachapoly_auth_decrypt(&ctx, data_len, nonce_buf, add_buf, add_len, tag,
					      data, output_buf);
	if (ret < 0) {
		goto cleanup;
	}
