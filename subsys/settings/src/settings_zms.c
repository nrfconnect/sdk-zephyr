...
static int settings_zms_load(struct settings_store *cs,
			     const struct settings_load_arg *arg);
static int settings_zms_save(struct settings_store *cs, const char *name,
			     const char *value, size_t val_len);
static void *settings_zms_storage_get(struct settings_store *cs);

static struct settings_store_itf settings_zms_itf = {
...
static ssize_t settings_zms_read_fn(void *back_end, void *data, size_t len)
{
	...
	rc = zms_read(rd_fn_arg->fs, rd_fn_arg->id, data, len);
	if (rc > (ssize_t)len) {
		/* zms_read signals that not all bytes were read
...
static void settings_zms_cache_add(struct settings_zms *cf, const char *name,
				   uint32_t name_id)
{
...
static uint32_t settings_zms_cache_match(struct settings_zms *cf, const char *name,
			     ...
			     const struct settings_load_arg *arg)
{
	...
	while (1) {
		...
		rc2 = zms_read(&cf->cf_zms, name_id + ZMS_NAME_ID_OFFSET,
			       &buf, sizeof(buf));
		...
		if ((rc1 <= 0) && (rc2 <= 0)) {
			...
			if (name_id == cf->last_name_id) {
				cf->last_name_id--;
				zms_write(&cf->cf_zms, ZMS_NAMECNT_ID,
					  &cf->last_name_id, sizeof(uint32_t));
			}
		...
		ret = settings_call_set_handler(
			name, rc2,
			settings_zms_read_fn, &read_fn_arg,
			(void *)arg);
		if (ret) {
			break;
		}
...
found:
	if (delete) {
		...
		if (name_id == cf->last_name_id) {
			cf->last_name_id--;
			rc = zms_write(&cf->cf_zms, ZMS_NAMECNT_ID,
				       &cf->last_name_id, sizeof(uint32_t));
			if (rc < 0) {
	...
	/* write the value */
	rc = zms_write(&cf->cf_zms, write_name_id + ZMS_NAME_ID_OFFSET,
		       value, val_len);
	if (rc < 0) {
		return rc;
	}
...
int settings_zms_backend_init(struct settings_zms *cf)
{
	...
	rc = zms_read(&cf->cf_zms, ZMS_NAMECNT_ID, &last_name_id,
		      sizeof(last_name_id));
	if (rc < 0) {
...
int settings_backend_init(void)
{
	...
	zms_sector_size = CONFIG_SETTINGS_ZMS_SECTOR_SIZE_MULT *
