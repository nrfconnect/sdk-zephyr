...
uint8_t lwm2m_firmware_get_update_state_with_instance(uint16_t obj_inst_id)
{
...
{
	return lwm2m_firmware_get_update_state_with_instance(0);
}

void lwm2m_firmware_set_update_state_with_instance(uint16_t obj_inst_id, uint8_t state)
{
	bool error = false;
	...
	update_state[obj_inst_id] = state;
	NOTIFY_OBSERVER(LWM2M_OBJECT_FIRMWARE_ID, obj_inst_id, FIRMWARE_STATE_ID);
	LOG_DBG("Update state = %d", update_state[obj_inst_id]);
}

void lwm2m_firmware_set_update_state(uint8_t state)
{
	lwm2m_firmware_set_update_state_with_instance(0, state);
}

uint8_t lwm2m_firmware_get_update_result_with_instance(uint16_t obj_inst_id)
{
...
{
	return lwm2m_firmware_get_update_result_with_instance(0);
}

void lwm2m_firmware_set_update_result_with_instance(uint16_t obj_inst_id, uint8_t result)
{
	uint8_t state;
	bool error = false;
	...
	switch (result) {
	case RESULT_DEFAULT:
		lwm2m_firmware_set_update_state_with_instance(obj_inst_id, STATE_IDLE);
		break;
	case RESULT_SUCCESS:
	...
	update_result[obj_inst_id] = result;
	NOTIFY_OBSERVER(LWM2M_OBJECT_FIRMWARE_ID, 0, FIRMWARE_UPDATE_RESULT_ID);
	LOG_DBG("Update result = %d", update_result[obj_inst_id]);
}

void lwm2m_firmware_set_update_result(uint8_t result)
{
	lwm2m_firmware_set_update_result_with_instance(0, result);
}
			    ...
			    bool last_block, size_t total_size)
{
	...
	state = lwm2m_firmware_get_update_state_with_instance(obj_inst_id);
	if (state == STATE_IDLE) {
		...
		if (data_len == 0U && state == STATE_DOWNLOADED) {
			...
			lwm2m_firmware_set_update_result_with_instance(obj_inst_id, RESULT_DEFAULT);
			return 0;
		}
	...
	if (ret >= 0) {
		if (last_block) {
			lwm2m_firmware_set_update_state_with_instance(obj_inst_id,
								      STATE_DOWNLOADED);
		}

		return 0;
	} else if (ret == -ENOMEM) {
		lwm2m_firmware_set_update_result_with_instance(obj_inst_id, RESULT_OUT_OF_MEM);
	} else if (ret == -ENOSPC) {
		lwm2m_firmware_set_update_result_with_instance(obj_inst_id, RESULT_NO_STORAGE);
		/* Response 4.13 (RFC7959, section 2.9.3) */
		/* TODO: should include size1 option to indicate max size */
		ret = -EFBIG;
	} else if (ret == -EFAULT) {
		lwm2m_firmware_set_update_result_with_instance(obj_inst_id,
							       RESULT_INTEGRITY_FAILED);
	} else if (ret == -ENOMSG) {
		lwm2m_firmware_set_update_result_with_instance(obj_inst_id, RESULT_UNSUP_FW);
	} else {
		lwm2m_firmware_set_update_result_with_instance(obj_inst_id, RESULT_UPDATE_FAILED);
	}
...
#ifdef CONFIG_LWM2M_FIRMWARE_UPDATE_PULL_SUPPORT
	uint8_t state = lwm2m_firmware_get_update_state_with_instance(obj_inst_id);
			      ...
			      uint8_t *args, uint16_t args_len)
{
	...
	lwm2m_firmware_set_update_state_with_instance(obj_inst_id, STATE_UPDATING);
...
static struct lwm2m_engine_obj_inst *firmware_create(uint16_t obj_inst_id)
{
	...
	INIT_OBJ_RES_OPT(FIRMWARE_PACKAGE_NAME_ID, res[obj_inst_id], i, res_inst[obj_inst_id], j, 1,
			 false, true, NULL, NULL, NULL, NULL, NULL);
	INIT_OBJ_RES_OPT(FIRMWARE_PACKAGE_VERSION_ID, res[obj_inst_id], i, res_inst[obj_inst_id], j,
			 1, false, true, NULL, NULL, NULL, NULL, NULL);
...
#endif
		LOG_INF("Creating LWM2M FW instance %d", idx);
