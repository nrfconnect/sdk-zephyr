		    ...
		    void *cb_arg)
{
	...
	/* Migrate `bt_keys` stored with extra data to the new format in the most common scenario
	 * where both legacy pairing and signing support are disabled. This allows to still properly
	 * load and use the keys after firmware upgrade.
	 */
		...
		if (len >= load_size) {
			...
			memcpy((char *)keys + offsetof(struct bt_keys, enc_size), val, load_size);
			/* Ensure no unsupported key types are set. */
			keys->keys &= (BT_KEYS_IRK | BT_KEYS_LTK_P256);
			/* Mark the keys as valid only if they contain an LTK. */
