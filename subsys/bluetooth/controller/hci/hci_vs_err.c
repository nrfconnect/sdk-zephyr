...
static struct net_buf *vs_err_evt_create(uint8_t subevt, uint8_t len)
{
	...
	/* The callers report a fatal error with interrupts locked, and may do so from an interrupt
	 * or a zero-latency interrupt context. Waiting is not an option there: nothing can return a
	 * buffer to the pool while interrupts are locked, and blocking in an interrupt context is
	 * not allowed. The pool is reserved for this purpose and the system does not continue past
	 * the fatal error, so a single attempt is enough.
	 */
	buf = net_buf_alloc(&vs_err_tx_pool, K_NO_WAIT);
	if (buf) {
					       ...
					       uint32_t line, uint64_t pc)
{
	...
	if (file_path) {
		...
		if (file_name_len) {
			...
			LOG_ERR("Not a file name");
		}
	} else {
		LOG_ERR("Not a file path");
	}
