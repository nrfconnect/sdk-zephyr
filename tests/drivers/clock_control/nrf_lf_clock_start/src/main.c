...
static void synth_check(bool on, nrf_clock_lfclk_t type)
{
	...
	(DT_STRING_TOKEN(DT_NODELABEL(lfclk), k32src) == CLOCK_CONTROL_NRF_K32SRC_SYNTH))
	if (!IS_ENABLED(CONFIG_SYSTEM_CLOCK_NO_WAIT)) {
		zassert_true(on, "Clock should be on");
...
#else
	xtal = (DT_STRING_TOKEN(DT_NODELABEL(lfclk), k32src) == CLOCK_CONTROL_NRF_K32SRC_XTAL) ||
	       (DT_STRING_TOKEN(DT_NODELABEL(lfclk), k32src) ==
...
#else
	} else if (DT_STRING_TOKEN(DT_NODELABEL(lfclk), k32src) == CLOCK_CONTROL_NRF_K32SRC_RC) {
#endif
...
void *test_init(void)
{
	...
	TC_PRINT("CLOCK_CONTROL_NRF_K32SRC=%s\n",
		(DT_STRING_TOKEN(DT_NODELABEL(lfclk), k32src) ==
		CLOCK_CONTROL_NRF_K32SRC_RC)    ? "RC" :
		(DT_STRING_TOKEN(DT_NODELABEL(lfclk), k32src) ==
		CLOCK_CONTROL_NRF_K32SRC_SYNTH) ? "SYNTH" :
		(DT_STRING_TOKEN(DT_NODELABEL(lfclk), k32src) ==
		CLOCK_CONTROL_NRF_K32SRC_XTAL)  ? "XTAL"
						: "???");
#endif
	if (IS_ENABLED(CONFIG_SYSTEM_CLOCK_NO_WAIT)) {
