...
ZTEST(nrf_grtc_timer, test_timer_count_in_compare_mode)
{
	...
	char user_data[] = "test_timer_abort_in_compare_mode test\n";
	...
	key = z_nrf_grtc_timer_compare_int_lock(channel);
	test_ticks =
		z_nrf_grtc_timer_read() + z_nrf_grtc_timer_get_ticks(K_MSEC(TIMER_COUNT_TIME_MS));
	err = z_nrf_grtc_timer_set(channel, test_ticks, timer_compare_interrupt_handler,
				   (void *)user_data);
	z_nrf_grtc_timer_compare_int_unlock(channel, key);

	zassert_true(K_TIMEOUT_EQ(K_TICKS(z_nrf_grtc_timer_compare_read(channel)),
				  K_TICKS(test_ticks)),
...
ZTEST(nrf_grtc_timer, test_timer_abort_in_compare_mode)
{
	...
	compare_isr_call_counter = 0;
	z_nrf_grtc_timer_compare_int_lock(channel);
	test_ticks =
		z_nrf_grtc_timer_read() + z_nrf_grtc_timer_get_ticks(K_MSEC(TIMER_COUNT_TIME_MS));
	err = z_nrf_grtc_timer_set(channel, test_ticks, timer_compare_interrupt_handler,
				   (void *)user_data);
