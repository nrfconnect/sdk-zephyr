...
# Verify that re-connection with 2 bonded devices works using directed advertisements where
...
# 8. Peripheral starts advertising again with the first identity and central
# identity as a peer address.
# 9. Central connects to the peripheral and encripts the link.
# 10. Central disconnects.
# 11. Peripheral starts advertising with the second identity and central identity as a peer address.
...
simulation_id="security_bond_keep_dir"
verbosity_level=2
...
Execute "$central_exe" \
    -v=${verbosity_level} -s=${simulation_id} -d=0 -testid=central_dir -RealEncryption=1

Execute "$peripheral_exe" \
    -v=${verbosity_level} -s=${simulation_id} -d=1 -testid=peripheral_dir -RealEncryption=1
