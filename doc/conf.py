...
# -- Partial documentation build ------------------------------------------
#
# The SKIP_* switches (see doc/Makefile; the ``html-minimal`` target enables
# them all) set these tags to skip expensive autogenerated layers (Doxygen,
# Kconfig and external board/sample/snippet content) when previewing
# narrative changes. The options below turn the matching generators off; the
# zephyr.partial_build extension degrades the content they would have
# produced (placeholder directives, plain-text references) so the build still
# passes under -W. Such a build is preview-only: it can neither validate
# cross-references nor be published.
SKIP_DOXYGEN, SKIP_KCONFIG, SKIP_EXTERNAL_CONTENT = (
    tags.has(t)  # pylint: disable=undefined-variable  # noqa: F821
    for t in ("skip_doxygen", "skip_kconfig", "skip_external_content")
)
...
if SKIP_EXTERNAL_CONTENT:
    # the board/sample/snippet destination pages are not built here
