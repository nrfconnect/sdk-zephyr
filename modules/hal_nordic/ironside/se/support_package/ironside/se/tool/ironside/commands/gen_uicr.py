...
try:
    ...
    sys.exit("Missing dependency 'pyelftools'")
try:
...
def add_parser(subparsers: SubParsers) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Automatically reallocate IPCMAP entries, under the assumption that each ELF contains "
            "IPCMAP entries starting from index 0. "
        ),
    )
    parser.add_argument(
        "--out-merged-hex",
...
def cmd_handler(args: argparse.Namespace) -> None:
    try:
        ...
        if (
            args.in_periphconf_elfs or args.in_secondary_periphconf_elfs
        ) and args.periphconf_section_name is None:
            raise ScriptError(
                "--periphconf-section-name is required when "
...
def extract_and_combine_periphconfs(
    elf_files: list[argparse.FileType],
    ...
    for in_file in elf_files:
        elf = ELFFile(in_file)
        conf_section = elf.get_section_by_name(periphconf_section_name)
...
# Adjusts all IPCMAP entries found in the periphconf by the given index offset, used as a
# workaround to allow each PERIPHCONF to allocate entries starting from index 0.
#
