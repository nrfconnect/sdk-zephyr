...
static inline void qspi_lock(const struct device *dev)
{
	 ...
	 */
#if defined(CONFIG_SOC_SERIES_NRF53)
	static const uint32_t base_clock_frequency = BASE_CLOCK_FREQUENCY;
...
static inline void qspi_unlock(const struct device *dev)
{
	 ...
	 */
	static const uint32_t default_base_clock_frequency = DEFAULT_BASE_CLOCK_FREQUENCY;

	clock_control_set_rate(DEVICE_DT_GET_ONE(nordic_nrf_clock_hfclk192m), NULL,
			       &default_base_clock_frequency);
	k_busy_wait(BASE_CLOCK_SWITCH_DELAY_US);
#endif
