			 ...
			 lv_coord_t y, lv_color_t color, lv_opa_t opa)
{
	...
	uint8_t *buf_xy;
	uint8_t bit;
	lv_coord_t width = data->cap.x_resolution;
	lv_coord_t height = data->cap.y_resolution;
	if (data->cap.screen_info & SCREEN_INFO_MONO_VTILED) {
	...
	} else if (data->cap.screen_info & SCREEN_INFO_MONO_V_BITMAP) {

		if (data->cap.screen_info & SCREEN_INFO_MONO_ROTATED_180) {
			buf_xy = buf + x * height / 8 + (height - 1 - y) / 8;
			if (data->cap.screen_info & SCREEN_INFO_MONO_MSB_FIRST) {
				bit = y % 8;
			} else {
				bit = 7 - (y % 8);
			...
			buf_xy = buf + (width - 1 - x) * height / 8 + y / 8;
			if (data->cap.screen_info & SCREEN_INFO_MONO_MSB_FIRST) {
				bit = 7 - y % 8;
			} else {
				bit = y % 8;
		...
		if (data->cap.screen_info & SCREEN_INFO_MONO_ROTATED_180) { // TODO
			buf_xy = buf + (width - 1 - x) / 8 + (height - y - 1) * width / 8;
			if (data->cap.screen_info & SCREEN_INFO_MONO_MSB_FIRST) {
				bit = x % 8;
			} else {
				bit = 7 - x % 8;
...
void lvgl_rounder_cb_mono(lv_disp_drv_t *disp_drv, lv_area_t *area)
{
	...
	} else if (data->cap.screen_info & SCREEN_INFO_Y_ALIGNMENT_WIDTH) {
		area->y1 = 0;
		area->y2 = data->cap.y_resolution - 1;
	} else {
		if (data->cap.screen_info & SCREEN_INFO_MONO_VTILED) {
			area->y1 &= ~0x7;
			area->y2 |= 0x7;
