...
void main(void)
{
	...
	printk("Internal Trusted Storage sample started.\n");
	printk("PSA Internal Trusted Storage API Version %d.%d\n",
			PSA_ITS_API_VERSION_MAJOR, PSA_ITS_API_VERSION_MINOR);

	printk("Writing data to UID1: %s\n", TEST_STRING_1);
	psa_storage_uid_t uid1 = 1;
	psa_storage_create_flags_t uid1_flag = PSA_STORAGE_FLAG_NONE;

	status = psa_its_set(uid1, sizeof(TEST_STRING_1), TEST_STRING_1, uid1_flag);
	...
	status = psa_its_get_info(uid1, &uid1_info);
	if (status != PSA_SUCCESS) {
	...
	status = psa_its_get(uid1, 0, sizeof(TEST_STRING_1), &stored_data, &bytes_read);
	if (status != PSA_SUCCESS) {
	...
	status = psa_its_set(uid1, sizeof(TEST_STRING_2), TEST_STRING_2, uid1_flag);
	if (status != PSA_SUCCESS) {
	...
	status = psa_its_set(uid2, sizeof(TEST_STRING_1), TEST_STRING_1, uid2_flag);
	if (status != PSA_SUCCESS) {
	...
	status = psa_its_set(uid2, sizeof(TEST_STRING_2), TEST_STRING_2, uid2_flag);
